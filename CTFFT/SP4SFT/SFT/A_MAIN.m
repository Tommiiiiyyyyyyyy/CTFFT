...
if check_flatwinresl == 2%存在する場合、読み込み。
    ...
    filename_fw_l = ['flatwin', filename_prm, '.mat'];% 時間領域の flat window function のファイル名．
...
n_absX_hat_d = absX_hat_d/max(absX_hat_d);%正規化
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%　パラメータおよび結果を構造体と個別にそれぞれ保存
foldername = [datestr(now, 'yyyymmdd_HHMMSS'),'_results'];%時刻を取得し，結果を保存するフォルダの名前に加える．
filename_str = 'result_str.mat';
...
cd(SR);%保存用フォルダに移動
mkdir(foldername);%シミュレーション毎の保存用フォルダの作成
cd(foldername);%前行で作ったフォルダに移動
