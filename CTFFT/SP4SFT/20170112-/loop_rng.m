...
% outer loop 4
for l = 1:L
    % inner loop; estimation loop; Estimation loops: for i Å∏ I, estimate ?xi as ?xÅå i = ?zhÉ–(i)É÷É—i/?GoÉ–(i).
    for k = 1:n
    if I_d(k) == 1;
...
f = n*(0:(n/2))/n;
% plot(PLOT)
plot(PLOT(:,1),'r-.','LineWidth',3);
hold on;plot(PLOT(:,2:N));
legend('original')
%plot(f, PLOT2)
